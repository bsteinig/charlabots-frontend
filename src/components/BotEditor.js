...
function BotEditor({ formHandler, name, description, code, isEditing }) {
  ...
  return (
    <form onSubmit={form.onSubmit((values) => formHandler(values, editor.getText()))}>
      <Container size="lg">
        <TextInput
        ...
        <Text size="lg">{ isEditing ? `Edit ${form.values.name}'s Code Below` : 'Add Code Below'}</Text>
        <RichTextEditor
          editor={editor}
